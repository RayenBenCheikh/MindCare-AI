...
export const getDetailedSleepDisplay = (sleepQuality: string, assessments: any[]): { display: string; isAverage: boolean } => {
    if (sleepQuality === 'Average' && assessments.length > 0) {
        ...
        if (sleepResponses.length > 0) {
            const average = sleepResponses.reduce((sum, score) => sum + score, 0) / sleepResponses.length;
...
export const calculateStreakDays = (assessments: any[]): number => {
    ...
    const uniqueDates = [...new Set(assessments.map(a =>
    ...
    for (let i = 0; i < uniqueDates.length; i++) {
        ...
        const daysDifference = Math.floor((checkDate.getTime() - assessmentDate.getTime()) / (1000 * 60 * 60 * 24));
...
export const getStreakDetails = (assessments: any[]): { streak: number; streakData: boolean[] } => {
...
export const processTrackerData = (assessments: any[], conversations: any[]): MindfulTrackerData => {
    const now = new Date();
    ...
    if (latestAssessment?.responses?.[2]) {
        ...
        if (sleepResponses.length > 0) {
            const sleepScore = sleepResponses.reduce((sum, score) => sum + score, 0) / sleepResponses.length;
